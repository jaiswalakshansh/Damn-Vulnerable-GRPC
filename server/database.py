...
from server.config import (
    DB_PATH, ADMIN_USERNAME, ADMIN_PASSWORD, ADMIN_EMAIL, FLAGS
)
...
def _seed_data(conn: sqlite3.Connection) -> None:
    ...
    if admin:
        cursor.execute(
            "INSERT OR IGNORE INTO notes (id, title, content, owner_id, is_private) "
            "VALUES (1,?,?,?,1)",
            ("Admin Secret Note", f"IDOR Flag: {FLAGS['idor']}", admin["id"])
        )
        cursor.execute(
            "INSERT OR IGNORE INTO notes (title, content, owner_id, is_private) "
            "VALUES (?,?,?,0)",
    ...
    from server.config import JWT_SECRET, INTERNAL_SERVICE_VALUE
    secrets = [
